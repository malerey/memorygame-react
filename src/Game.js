...
class Game extends Component {
  ...
  render() {
    ...
    let gameStatus = <div className='Game-status'>
                      <div>Jugadas: {this.state.turnNo}</div>
    ...
    if (this.state.pairsFound === this.memoryCards.NUM_IMAGES) {
      gameStatus = <div className='Game-status'>
                    <div>Ganaste!</div>
                    <div>Usaste {this.state.turnNo-1} jugadas</div>
                    <div><button onClick={this.onPlayAgain}>Jugar de nuevo</button></div></div>;      
    }

    return (
      <div className='Game'>
        <header className='Game-header'>
          <div className='Game-title'>Un memotest en React</div>
        </header>
        <div>
          {gameStatus}
        </div>
